...
CREATE SEQUENCE "T_SCH_TASK_LOG_SEQ" INCREMENT BY 1 START WITH 1;
...
CREATE TABLE T_SCH_TASK (
  ...
  C_SCH_TASK_PARAM_JSON CLOB NOT NULL DEFAULT '{}',
...
CREATE TABLE T_REMOTE_JOB (
  ...
  C_REQUEST_URL CLOB NOT NULL,
  C_REQUEST_METHOD VARCHAR(10) NOT NULL,
  C_USER_AUTH_STRING VARCHAR(255) NOT NULL,
  C_POST_JSON CLOB NOT NULL DEFAULT '{}',
  CONSTRAINT I_SCH_TASK_ID_UC UNIQUE (C_SCH_TASK_ID)
);

CREATE TABLE T_SCH_TASK_LOG (
  C_SCH_TASK_LOG_ID SERIAL NOT NULL PRIMARY KEY,
  C_EXECUTION_TIME TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  C_EXECUTION_STATUS VARCHAR(10) NOT NULL,
  C_REMOTE_JOB_PARAMETERS_JSON CLOB NOT NULL DEFAULT '{}',
  CHECK (C_EXECUTION_STATUS in ('SUCCESS', 'FAILED'))
);